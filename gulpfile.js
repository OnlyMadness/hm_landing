...
function styles() {
    return src('app/' + preprocessor + '/' + filescss +  '.' + preprocessor + '') // Выбираем источник: "app/scss/main.scss" или "app/less/main.less"
        .pipe(plumber())
        .pipe(eval('sass')()) // Преобразуем значение переменной "preprocessor" в функцию
        .pipe(concat(filescss + '.css')) // Конкатенируем в файл app.min.css
        .pipe(csscomb())
